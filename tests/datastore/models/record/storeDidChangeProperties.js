...
function _expect(fooObject, expectedStatusCnt, expectedFooCnt) {
...
test("changing attributes on a parent store should notify child store if inherited", function() {
  ...
  _expect(childfoo,1,1); // should reflect on child
});
...
test("changing attributes on a parent store should NOT notify child store if locked", function() {
  ...
  parentfoo.writeAttribute('foo', 'baz'); // must not be bar
  SC.RunLoop.end();
  _expect(childfoo,0,0); // should not reflect on child
  _expect(parentfoo,2,1);
  ...
  _expect(childfoo,1,1); //the childfoo record is reset to whatever the parentValue is.
  SC.RunLoop.end();
